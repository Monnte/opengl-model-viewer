...
void DrawSkyboxCombo(Skybox *&currentSkybox, std::vector<string> &skyboxNames,
                     std::vector<string> &skyboxPaths, int *currentName, unsigned int *irradianceMap,
                     unsigned int *prefilterMap, Shader &irradianceShader, Shader &prefilterShader);
...
int main()
{
    ...
    Shader irradianceShader((parentDir + shaderPath + "cubemap.vs").c_str(), (parentDir + shaderPath + "irradiance_convolution.fs").c_str());
    Shader prefilterShader((parentDir + shaderPath + "cubemap.vs").c_str(), (parentDir + shaderPath + "prefilter.fs").c_str());
    Shadow shadow;
    ...
    glUniform1i(glGetUniformLocation(shaderProgram.ID, "shadowMap"), 4);
    glUniform1i(glGetUniformLocation(shaderProgram.ID, "irradianceMap"), 5);
    glUniform1i(glGetUniformLocation(shaderProgram.ID, "prefilterMap"), 6);
    glUniform1i(glGetUniformLocation(shaderProgram.ID, "brdfLUT"), 7);
    ...
    unsigned int brdfLUTTexture, irradianceMap, prefilterMap;
    brdfLUTTexture = loadBRDFLUT((parentDir + texturePath + "brdf.png").c_str());
    precomputeIBL(&irradianceMap, &prefilterMap, skybox.textureID, irradianceShader, prefilterShader);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        ImGui::Separator();
        DrawSkyboxCombo(currentSkybox, SkyboxNames, SkyboxPaths, &currentSkyboxIndex, &irradianceMap, &prefilterMap, irradianceShader, prefilterShader);
...
void DrawSkyboxCombo(Skybox *&currentSkybox, std::vector<string> &skyboxNames, std::vector<string> &skyboxPaths, int *currentName, unsigned int *irradianceMap,
                     unsigned int *prefilterMap, Shader &irradianceShader, Shader &prefilterShader)
{
            ...
            (void *)&skyboxNames, skyboxNames.size()))
    {
        ...
        precomputeIBL(irradianceMap, prefilterMap, currentSkybox->textureID, irradianceShader, prefilterShader);
    }
}
